package com.mlijo.aryaym.konsumen_mlijo.Obrolan;
...
public class DaftarObrolanAdapter extends RecyclerView.Adapter<DaftarObrolanAdapter.DaftarObrolanViewHolder>{
    ...
    public void onBindViewHolder(final DaftarObrolanViewHolder holder, int position) {
        try {
            ...
            //load data penerima
            mDatabase.child(Constants.PENJUAL).child(penerimaId).addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot != null){
                        final PenjualModel penjualModel = dataSnapshot.getValue(PenjualModel.class);
                        if (penjualModel != null){
                            holder.txtNamaPenerima.setText(penjualModel.getDetailPenjual().get(Constants.NAMA).toString());
                            ImageLoader.getInstance().loadImageAvatar(activity, penjualModel.getDetailPenjual().get(Constants.AVATAR).toString(), holder.imgAvatar);
                        }
                        //event click
